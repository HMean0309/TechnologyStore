...
import GUI.Components.BuildTable;
import GUI.Main;
...
public class TaoPhieuNhap extends javax.swing.JPanel implements MouseListener {
    ...
    public TaoPhieuNhap() {
        ...
        ResultSet rs = phieuNhapDAO.getLatestMaPhieuNhap();
        int soLuongPhieuNhapDaTao = 0;
        try {
            if (rs.next()) {
                soLuongPhieuNhapDaTao = rs.getInt(1);
            }
            rs.close();
...
//        txtmaphieunhap.setText("PN" + maPhieuNhap);
        ...
        LinkedHashSet<SanPhamDTO> list = sanPhamPhieuNhapBUS.getAllSanPham();
    ...
    public TaoPhieuNhap(TaiKhoanDTO taiKhoanDTO) {
        ...
        txtnhanviennhap.setText(nhanVienBUS.selectByID(taiKhoanDTO.getIdNV()).getHoten());
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == tblsoluongsanpham) {
            ...
            if (selectedRow != -1) {
                ...
                if (result != null) {
                    txtmasanpham.setText(String.valueOf(result.getId()));
                    txttensanpham.setText(result.getName());
                } else {
...
//        model.setRowCount(0); // Xóa hết dữ liệu trong bảng
    ...
    private void initComponents() {
        ...
        tblthongtinspdathem.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "STT", "Mã SP", "Tên sản phẩm", "Màu", "Loại", "Seri", "Giá nhập"
            }
        ));
    ...
    private boolean createPhieuNhap() {
        try {
            ...
            PhieuNhapKhoDTO pn = new PhieuNhapKhoDTO(maphieunhap, timestamp, mancc, manv, tongtien, 1);
    ...
    private void addChiTietPhieuNhapToDatabase() {
        ...
        ArrayList<ChiTietNhapKhoDTO> chiTietPhieuNhapList = new ArrayList<>();
        ...
        for (int i = 0; i < model.getRowCount(); i++) {
            ...
            try {
                ...
                ChiTietNhapKhoDTO chiTietPhieuNhapDTO = new ChiTietNhapKhoDTO(maphieunhap, masp, soluong, gianhap, giaxuat);
    ...
    public void ThemSanPhamVaoHangCho() {
        if (txtmasanpham.getText().isEmpty()) {
            ...
            tblModel = (DefaultTableModel) tblthongtinspdathem.getModel();
            LinkedHashSet<SanPhamDTO> newProductSet = new LinkedHashSet<>(sanPhamPhieuNhapDAO.getListSanPham(masp));
            updatetableaddedproducts(new ArrayList<>(newProductSet), tblthongtinspdathem);
        }
    }
    ...
    public void updateInputTable(LinkedHashSet<PhieuNhapKhoDTO> danhSachPhieuNhap) {
        PhieuNhap phieunhap = new PhieuNhap();
        nhaCungCapDAO = new NhaCungCapDAO();
        DefaultTableModel model = (DefaultTableModel) phieunhap.tblPhieuNhap.getModel();
        model.setRowCount(0);
        int stt = 1;
        for (PhieuNhapKhoDTO pn : danhSachPhieuNhap) {
            String TenNcc = nhaCungCapDAO.selectById(pn.getIdNCC()).getTenncc();
            model.addRow(new Object[] {
                stt++,
                pn.getId(),
                TenNcc,
    ...
    public String SelectedMaNCC(String TenNccCanTim) {
        LinkedHashSet<NhaCungCapDTO> nhaCungCapSet = new LinkedHashSet<>(nhaCungCapBUS.getAllNhaCungCap());
        String maNhaCungCap = null; // Giá trị mặc định nếu không tìm thấy

        for (NhaCungCapDTO nhaCungCap : nhaCungCapSet) {
            if (nhaCungCap.getName().equals(TenNccCanTim)) {
                maNhaCungCap = nhaCungCap.getId();
                break;
            }
        }
