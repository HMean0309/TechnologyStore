...
public class writePDF {
    ...
    public void writePN(PhieuNhapDTO pn, DefaultTableModel tblModel) {
        String url = "";
        try {
            fd.setTitle("In phiếu nhập");
            fd.setLocationRelativeTo(null);
            url = getFile(pn.getId() + "");
            ...
            Paragraph paragraph1 = new Paragraph("Mã phiếu: " + pn.getId(), fontNormal10);
            ...
            paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("Mã nhân viên: " + pn.getIdNhanVien(), fontNormal10));
            Paragraph paragraph4 = new Paragraph("Thời gian nhập: " + Formater.formatTime(pn.getNgayNhap()), fontNormal10);
    ...
    */
    public void writeHD(HoaDonDTO hd, DefaultTableModel tblModel) {
        String url = "";
        try {
            fd.setTitle("In hóa đơn");
            fd.setLocationRelativeTo(null);
            url = getFile(hd.getId() + "");
            if (url.equals("nullnull")) {
                return;
            }
            url = url + ".pdf";
            ...
            // Thêm dòng Paragraph vào file PDF
            Paragraph paragraph1 = new Paragraph("Mã phiếu: " + hd.getId(), fontNormal10);
            String kh = hd.getNameKhachHang();
            Paragraph paragraph2 = new Paragraph("Khách hàng: " + kh, fontNormal10);
            paragraph2.add(new Chunk(createWhiteSpace(5)));
            paragraph2.add(new Chunk("-"));
            paragraph2.add(new Chunk(createWhiteSpace(5)));
            String diachikh = KhachHangDAO.getInstance().selectById(hd.getIdKhachHang() + "").getFullAddress();
            paragraph2.add(new Chunk(diachikh, fontNormal10));

            String ngtao = NhanVienDAO.getInstance().selectById(hd.getIdNhanVien() + "").getName();
            Paragraph paragraph3 = new Paragraph("Người thực hiện: " + ngtao, fontNormal10);
            paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("-"));
            paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("Mã nhân viên: " + hd.getIdNhanVien(), fontNormal10));
            Paragraph paragraph4 = new Paragraph("Thời gian lập: " + Formater.formatTime(hd.getNgayLap()), fontNormal10);
            ...
            // Thêm tổng thành tiền vào PDF
            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Tổng thành tiền: " + formatter.format(hd.getOrderAmount()) + "đ", fontBold15));
            paraTongThanhToan.setIndentationLeft(300);
            document.add(paraTongThanhToan);
            document.add(Chunk.NEWLINE);
            document.add(Chunk.NEWLINE);
    ...
    public void writeBH(BaoHanhDTO bh, DefaultTableModel tblModel) {
        String url = "";
        try {
            fd.setTitle("In bảo hành");
            fd.setLocationRelativeTo(null);
            url = getFile(bh.getId() + "");
            if (url.equals("nullnull")) {
                return;
            }
            url = url + ".pdf";
            ...
            // Thêm tên công ty vào file PDF
            document.add(Chunk.NEWLINE);
            Paragraph header = new Paragraph("THÔNG TIN PHIẾU BẢO HÀNH", fontBold25);
            header.setAlignment(Element.ALIGN_CENTER);
            document.add(header);

            // Thêm dòng Paragraph vào file PDF
            Paragraph paragraph0 = new Paragraph("Mã phiếu: " + bh.getId(), fontNormal10);
            Paragraph paragraph1 = new Paragraph("Hoá đơn áp dụng: " + bh.getIdHoaDon(), fontNormal10);
            String kh = bh.getNameKhachHang();
            Paragraph paragraph2 = new Paragraph("Khách hàng: " + kh, fontNormal10);
            paragraph2.add(new Chunk(createWhiteSpace(5)));
            paragraph2.add(new Chunk("-"));
            paragraph2.add(new Chunk(createWhiteSpace(5)));
            String diachikh = KhachHangDAO.getInstance().selectById(bh.getIdKhachHang() + "").getFullAddress();
            paragraph2.add(new Chunk(diachikh, fontNormal10));

            String ngtao = NhanVienDAO.getInstance().selectById(bh.getIdNhanVien() + "").getName();
            Paragraph paragraph3 = new Paragraph("Người thực hiện: " + ngtao, fontNormal10);
            paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("-"));
            paragraph3.add(new Chunk(createWhiteSpace(5)));
            paragraph3.add(new Chunk("Mã nhân viên: " + bh.getIdNhanVien(), fontNormal10));
            Paragraph paragraph4 = new Paragraph("Thời gian lập: " + Formater.formatTime(bh.getNgayBaoHanh()), fontNormal10);
            ...
            table.setWidthPercentage(100);
            table.setWidths(new float[]{ 30f, 20f, 20f });

            // Thêm tiêu đề cột
            table.addCell(new PdfPCell(new Phrase("Tên sản phẩm", fontBold15)));
            table.addCell(new PdfPCell(new Phrase("Màu sắc", fontBold15)));
            table.addCell(new PdfPCell(new Phrase("Số lượng bảo hành", fontBold15)));
            ...
            // Thêm tổng thành tiền vào PDF
            Paragraph paraTongThanhToan = new Paragraph(new Phrase("Ngày trả hàng: " + Formater.formatDate(bh.getNgayTraHang()), fontBold15));
            paraTongThanhToan.setIndentationLeft(300);
            document.add(paraTongThanhToan);
            document.add(Chunk.NEWLINE);
            document.add(Chunk.NEWLINE);
